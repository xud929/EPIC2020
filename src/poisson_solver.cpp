...
double Poisson_Solver::soft_gaussian::slice_slice_kick(std::vector<double> &coord1, const std::vector<double> &weight, double S1,
        const std::array<double,10> &params2, double S2, const std::tuple<unsigned,double> &info) const{
    ...
    */
    ...
    for(unsigned i=0;i<n1;++i){
        ...
        pz-=(Uxx*dsigx2_dS2_half+Uyy*dsigy2_dS2_half)*dS2_dz;
#endif
        x-=S1*px;y-=S1*py;
#ifdef HIRATA_MAP
        pz+=0.25*(px*px+py*py);
#endif
        x*=weight[i];px*=weight[i];y*=weight[i];py*=weight[i];pz*=weight[i];
        if(which==gaussian_when_luminosity){
            ret+=expterm*weight[i];
        }
    }
    /*
    int rank=-1;
...
double Poisson_Solver::soft_gaussian::operator()(std::vector<double> &coord1, const std::array<double,4> &param1, MPI_Comm comm1,
    ...
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);
    if(rank==0){
        //std::cout<<coord1[0]<<"\t"<<coord2[0]<<std::endl;
        std::cout<<"mx1,mx2,my1,my2 = ("<<m_x1<<","<<m_x2<<","<<m_y1<<","<<m_y2<<")"<<std::endl;
        std::cout<<"mpx1,mpx2,mpy1,mpy2 = ("<<m_px1<<","<<m_px2<<","<<m_py1<<","<<m_py2<<")"<<std::endl;
        std::cout<<"sx1,sx2,sy1,sy2 = ("<<s_x1<<","<<s_x2<<","<<s_y1<<","<<s_y2<<")"<<std::endl;
        std::cout<<"spx1,spx2,spy1,spy2 = ("<<s_px1<<","<<s_px2<<","<<s_py1<<","<<s_py2<<")"<<std::endl;
        std::cout<<"sx1px1,sx2px2,sy1py1,sy2py2 = ("<<s_x1_px1<<","<<s_x2_px2<<","<<s_y1_py1<<","<<s_y2_py2<<")"<<std::endl;
        std::cout<<lb1<<"\t"<<c1<<"\t"<<rb1<<std::endl;
        std::cout<<lb2<<"\t"<<c2<<"\t"<<rb2<<std::endl;
    }
    */
    ...
    temp_luminosity=slice_slice_kick(L1,wL1,0.5*(lb1-c2),moments2,0.5*(c2-lb1),std::make_tuple(2U,w2));
    //if(rank==0){
        //std::cout<<temp_luminosity<<std::endl;
    //}
    if(gaussian_when_luminosity==2)
        slice_luminosity+=temp_luminosity;
    temp_luminosity=slice_slice_kick(R1,wR1,0.5*(rb1-c2),moments2,0.5*(c2-rb1),std::make_tuple(2U,w2));
    if(gaussian_when_luminosity==2)
        slice_luminosity+=temp_luminosity;

    // collision of beam2 (left and right boundary) with beam1 (center)
    std::vector<double> L2=coord2,R2=coord2;
    temp_luminosity=slice_slice_kick(L2,wL2,0.5*(lb2-c1),moments1,0.5*(c1-lb2),std::make_tuple(1U,w1));
    if(gaussian_when_luminosity==1)
        slice_luminosity+=temp_luminosity;
    temp_luminosity=slice_slice_kick(R2,wR2,0.5*(rb2-c1),moments1,0.5*(c1-rb2),std::make_tuple(1U,w1));
    if(gaussian_when_luminosity==1)
        slice_luminosity+=temp_luminosity;
